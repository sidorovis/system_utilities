	...
	namespace tests_
	{
		namespace common
		{
			...
			namespace details
			{
				...
				{
					static const boost::filesystem::path::string_type fs_separator( &boost::filesystem::path::preferred_separator );
					static const std::string separator( fs_separator.begin(), fs_separator.end() );
					static const boost::regex good_regex( "\\.\\" + separator + "+good_.+\\.ini" );
					boost::smatch result;
				...
				bool check_and_test_bad_file( const std::string& file_path )
				{
					static const boost::filesystem::path::string_type fs_separator( &boost::filesystem::path::preferred_separator );
					static const std::string separator( fs_separator.begin(), fs_separator.end() );
					static const boost::regex bad_regex( "\\.\\" + separator + "+bad_.+\\.ini" );
					boost::smatch result;
			...
			void property_reader_constructor_tests()
			{
				...
				for ( directory_iterator i(p) ; i != directory_iterator() ; ++i )
					...
					{
						const std::string path_str = i->path().string();
						if ( details::check_and_test_good_file( path_str ) )
							continue;
						if ( details::check_and_test_bad_file( path_str ) )
							continue;
